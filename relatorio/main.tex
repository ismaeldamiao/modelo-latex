...
%  Turorial LaTeX: https://ismaeldamiao.github.io/pt/pkg/latex/
...
\usepackage{tikz}                      % Para fazer fluxograma
% --------------------------------------
...
% Configuração do tikz para fluxograma
\usetikzlibrary{shapes.geometric, arrows}
...
\declaretheorem[style=definition,name=Defini\c{c}\~{a}o,parent=chapter,qed=\textemdash]{definicao}
...
% ------------------------------------------------------------------------------
   ...
   \chapter{Dados e análise}
      ...
      \begin{figure}[H]
         \centering
         \caption{\label{fig:fluxEuler}\footnotesize Exemplo de fluxograma
         para EDO de 1ª ordem.}
         \begin{tikzpicture}[node distance=2cm]
         \node (start) [startstop] {$f = f_0$; $x = x_0$};
         \node (open) [process, below of=start] {Abrir \textit{arquivo}};

         \node (for) [decision, below of=open] {$x \leq x_f$?};
         \node (pro1) [process, below of=for, yshift=-0.8cm] {$f = f + F(x, f) dx$};
         \node (pro2) [process, below of=pro1] {Escrever $x$ e $f$ em \textit{arquivo}};
         \node (pro3) [process, below of=pro2] {$x = x + dx$};

         \node (conec) [process, right of=for, xshift=3.5cm] {Fechar \textit{arquivo}};
         \node (stop) [startstop, below of=conec] {Fim};

         \draw [arrow] (start) -- (open);
         \draw [arrow] (open) -- (for);
         \draw [arrow] (for) -- node[anchor=east] {Sim} (pro1);
         \draw [arrow] (pro1) -- (pro2);
         \draw [arrow] (pro2) -- (pro3);
         \draw [arrow] (conec) -- (stop);
         \draw [arrow] (for) -- node[anchor=south] {Não} (conec);
         \path [line] (pro3) -- ++(-3.5,0) |- (for);
         \end{tikzpicture}
         \legend{\footnotesize Fonte: Autores.}
      \end{figure}
